...
dv.paragraph("### Filter search conditions through the dropdown box<br><br>")
...
> 		dv.paragraph(searchTypeDropdownMaker('searchType', dv.current().file.path))
		...
		if(searchType === "tags"){
			...
			dv.paragraph("Plese select a valid search type.")
		}
} else {
    dv.paragraph("<strong>!!! Please enable the MetaEdit plugin and then reload the document!</strong>")
}
...
Note: This script originates from Cuman's Blue Topaz example vault, a very valuable example library for learning. You can download this library at [this address](https://github.com/cumany/Blue-topaz-examples).