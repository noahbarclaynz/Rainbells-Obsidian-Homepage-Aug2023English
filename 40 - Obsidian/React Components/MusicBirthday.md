...
for (var ita=0;ita<files.length;ita++){
	...
	if (typeof(meta) != "undefined"){
		...
		if (birthday){
			...
			if (N < 31){
				count = count+1;
				cells.push(<div class = "birdcountdown">  <text class = "birdname"> {basename}Birthday Countdown </text><text class="countdowndays"> {N+1}days </text></div>) 
			}
		}
	}
}

const element = <div>In the next month, there are {count} birthdays: <br /> </div>;
